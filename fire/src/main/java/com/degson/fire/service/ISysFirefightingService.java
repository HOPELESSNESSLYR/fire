...
//    BufferedImage generateImage(String qrContent) throws WriterException;
    ...
    /**
     * Monthly summary per device group within [startDate, endDate]. Format yyyy-MM-dd.
     */
    List<MonthlySummaryVO> queryMonthlySummary(String startDate, String endDate);

    /**
     * 中文说明：
     * 在指定起止日期（yyyy-MM-dd）范围内，按设备组（灭火器/消防栓/阀组间/水泵房）汇总统计：
     * - totalCount：设备总数
     * - inspectedCount：该期间内存在检查记录的设备数量
     * - uninspectedCount：该期间内无检查记录的设备数量
     * - qualifiedCount：检查记录中合格（qualified='是'）的设备数量
     * - unqualifiedCount：检查记录中不合格（qualified='否'）的设备数量
     */
}
