<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="最新点检时间" prop="checkRecords">
        <el-input
    ...
    <el-table v-loading="loading" :data="firefightingList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="生产日期" align="center" prop="productDate" width="95">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.productDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="安装时间" align="center" prop="installTime" width="95">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.installTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="到期时间" align="center" prop="expiryDate" width="95">
        <template slot-scope="scope">
      ...
      <el-table-column label="最新检查时间" align="center" prop="checkRecords" width="95"/>
    ...
    async getList() {
      ...
      listFirefighting(this.queryParams).then(response => {
        ...
        for(var i=0;i<response.rows.length;i++){
          ...
          alert['qrContent'] = response.rows[i].qrContent;
          ...
          // console.log(this.fireIDNumber.indexOf(alert['fireId']) !== -1)
          ...
          }else if(staticDate < new Date().setMonth(new Date().getMonth() - 1)){
            ...
            // console.log(this.fireIDNumber.indexOf(anlert['fireId']) !== -1)
